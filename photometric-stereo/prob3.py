...
def pstereo_n(imgs, L, mask):
    ...
    grays = []
    for i in imgs:
        grays.append(i[:,:,0] + i[:,:,1] + i[:,:,2])
    # Save shape of original image for conversion in future
    sp = grays[0].shape
    # Flatten each image to its own row vector to allow for multiplication with L.T and eventually solving of eqn in form of Ax=B
    I = np.vstack(i.ravel() for i in grays)
    A = np.matmul(L.T, L)
    B = np.matmul(L.T, I)
    n = np.linalg.solve(A,B)
    # Get normal of n
    div = np.sqrt(np.sum((i**2) for i in n)).reshape(sp)
    # reshape each element in n to that of the original image
    n = [i.reshape(sp) for i in n]
    # normalize each element in n where the mask says data is valid, then stack the elements to color channels to form full image of normals 
    return np.stack([np.where(mask!=0, i/div, np.NaN) for i in n], axis=2)
...
def pstereo_alb(imgs, nrm, L, mask):
    # save original shape
    sp = imgs[0].shape
     # Flatten each image to its own row vector to allow for multiplication with L.T and eventually solving of for pseudoInverse
    I = np.vstack(i.ravel() for i in imgs)
    # M = (L.T * L)^-1 * L.T * I    where * is matrix multiplication
    #     (L.T * L)^-1 * L.T -> pseudo Inverse (inplemented manually instead of using np.linalg.pinv)
    M = np.matmul(np.matmul(np.linalg.inv(np.matmul(L.T,L)),L.T),I)
    # rho = norm(M)
    # compute rho and reshape to the shape of original image
    return np.sqrt(np.sum((i**2) for i in M)).reshape(sp)
